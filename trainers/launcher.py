...
from trainers import method_to_trainer
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="urbancars", choices=["urbancars", "celeba", "bar"]
    )
    ...
    parser.add_argument("--weight_decay2", type=float, default=1e-4)
...
def main():
    ...
    if args.slurm_partition is not None:
        ...
        job_list = []
        with executor.batch():
            for idx, job_args in enumerate(args_list):
                print("round:", idx)
                trainer = Trainer(job_args, logger)
