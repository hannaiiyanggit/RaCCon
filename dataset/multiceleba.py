...
class BiasedCelebA(Dataset):
    raw_celeba = tv_dataset.CelebA('D:\mywork\Whac-a-Mole\data', 'all')
    ...
    def __init__(self, root, target_name, biasA_name, biasB_name, biasA_ratio, biasB_ratio, split, transform=None):
        ...
        group_label, biasA_is_aligned, biasB_is_aligned = self._setup_group_label_is_aligned()
        self.group_label = group_label

        self.group_array = group_label.numpy() if torch.is_tensor(group_label) else np.array(group_label)

        self.biasA_is_aligned = biasA_is_aligned
        self.biasB_is_aligned = biasB_is_aligned

    def __getitem__(self, index, random_gap=False):
        ...
        if self.return_group_index:
            # 统一返回 group_index (全称)，适配我们之前写的兼容逻辑
            data_dict["group_index"] = int(self.group_array[index])

        if self.return_contrastive_pairs:
            rank = self.ranks[index]
            max_rank = int(self.max_rank[self.obj_label[index].item()])
            if random_gap:
                pos_rank = random.randint(min(rank+1, max_rank), max_rank)
                # pos_rank = random.randint(0, max_rank)
            else:
